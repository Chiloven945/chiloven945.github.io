---
title: 关于
menu:
    main: 
        weight: -90
        params:
            icon: user
---

<h3>幻想慘死於現實麾下，愛昰墓誌銘。</h3>

> <iframe style="border-radius:12px" src="https://open.spotify.com/embed/playlist/0qbtkkQOlo1xo8Z0Mhuzw2?utm_source=generator" width="100%" height="352" frameBorder="0" allowfullscreen="" allow="autoplay; clipboard-write; encrypted-media; fullscreen; picture-in-picture" loading="lazy"></iframe>

<h3>你可以称我为……</h3>

  - ***ClovenBugle***(9455) [^1]
  - ***Chiloven***(Bustiangle)***945***(5) [^2]
  - Mackenzie
  - chi
  - 阿奇

<h3>一些你可能感兴趣的……</h3>

  - 🐾 Furry
  - 🏳️‍🌈 性取向为泛性恋，不过偏向同性
  - 🌐 偏好语言：
    - 简体中文（母语）
    - 繁體中文（可理解）
    - English（熟练）
    - Ελληνικά（学习中）
    - 日本語（学习中）
  - 👤 MBTI 为 infp-t
  - 🗺️ 居住于 `中华人民共和国，重庆`

<h3>一些兴趣爱好……</h3>

  - 📓 写一些我喜欢题材的小说（可以看看我的博客文章或者 Pixiv！）；
  - 📖 也喜欢看一堆小说，Pixiv 重度患者（；
  - 🎮 喜欢玩 Minecraft 和 Cities Skylines （没错就这点……）；
  - 🖥️ 喜欢捣鼓一些计算机相关的东西；
  - 🅰️ 词源学，记点词根什么的；
  - 🏞️ 研究汉字字形构成；
  - 🎼 会做一些 MIDI，会扒一些谱；
  - 🎵 听点自己喜欢的音乐；
  - 📃 干点翻译工作，还能学习英语何乐而不为（；
  - 📺 看点知识性的视频；
  - 🐶 边牧是世界上最可爱的小狗；
  - 🐺 狼狼！狼狼好，狼狼可爱！！！

<h3>尝试做的事情……</h3>
  
  - 🧭 为一些 Java 版的模组制作翻译文件；
  - 💻 尝试学习 Java、Kotlin 与 C#；
  - 👥 交到更多朋友（快来和我扩列！）；
  - 🏛️ 学习希腊语；[^3]
  - 📚 完成 AP CSP/CSA 与 AP Calculus AB 的学习；
  - 😭 考雅思；
  - ❔ 扩充自己知识面的广度；
  - 😶‍🌫️ ~~与痛苦苟活于人类文明。~~

<h3>我的 Github Stats</h3>

<p><img src="https://github-readme-stats.vercel.app/api?username=Chiloven945&count_private=true&show_icons=true&theme=tokyonight" alt="chiloven945" /></p>
<p><img src="https://github-readme-stats.vercel.app/api/top-langs/?username=Chiloven945&layout=compact&theme=tokyonight" alt="chiloven945" /></p>

网站的声明：界面使用了 HarmonyOS Sans 字体。

[^1]: “ClovenBugle”实际上是一个 Xbox 自己生成的名字，“ChilovenBustiangle”则是其在字母上的变形；
[^2]: 后缀“945”最初并不存在，但因为名字的长度限制“9455”被强制缩短至“945”，并保留在了今天；
[^3]: 也许吧……可能吧……大概吧……
